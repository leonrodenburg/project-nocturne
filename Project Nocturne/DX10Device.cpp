...
/**
 * Constructor, creates a DirectX 10 device using the passed in
 * window and interface.
 *
 * @param	pWindow		The window to associate with
 */
NocturneDeviceDX10::Device::Device(NocturneCoreDX::Window* pWindow) : NocturneDeviceDX::Device()
{
 ...
 * Destructor.
 */
NocturneDeviceDX10::Device::~Device()
{
 ...
 */
PN_SWAPCHAIN_ID NocturneDeviceDX10::Device::CreateSwapChain(NocturneCoreDX::Window* pWindow)
{
	/* define result as 0 */
	...
	if(NULL != this->_pHandle)
	{
		...
		if(NULL != this->_pFactory)
		{
			...
			if(NULL != pSwapChain)
			{
				...
				ID3D10RenderTargetView* pRenderTargetView = this->_CreateRenderTargetView(this->_pHandle, pSwapChain);

				/* if the render target view is not null */
				if(NULL != pRenderTargetView)
				{
					/* create a depth stencil view */
					ID3D10DepthStencilView* pDepthStencilView = this->_CreateDepthStencilView(this->_pHandle, suitableMode.Width, suitableMode.Height);

					/* if the depth stencil view is not null */
					if(NULL != pDepthStencilView)
					{
						/* create a viewport */
						D3D10_VIEWPORT viewport = this->_CreateViewport(0, 0, suitableMode.Width, suitableMode.Height);
					...
					{
						/* throw exception */
						throw NocturneDevice::Exception("Failed to initialize depth stencil view in CreateSwapChain() in DX10Device");
					}
				}
				else
				{
					/* throw exception */
					throw NocturneDevice::Exception("Failed to initialize render target view in CreateSwapChain() in DX10Device");
				}
			}
			else
			{
				/* throw exception */
				throw NocturneDevice::Exception("Failed to initialize swap chain in CreateSwapChain() in DX10Device");
			}
		}
		else
		{
			/* throw exception */
			throw NocturneDevice::Exception("NULL IDXGIFactory passed to CreateSwapChain() in DX10Device");
		}
	}
	else
	{
		/* throw exception */
		throw NocturneDevice::Exception("Could not initialize swap chain using NULL D3D10 device in CreateSwapChain()");
	}
 ...
 */
void NocturneDeviceDX10::Device::ResizeSwapChain(PN_SWAPCHAIN_ID uiIdentifier, unsigned int uiWidth, unsigned int uiHeight)
{
	/* if there is a handle to the device */
	if(NULL != this->_pHandle)
	{
		/* unbind the resources from the device */
		this->_pHandle->OMSetRenderTargets(0, NULL, NULL);

		/* set the viewport to null */
		this->_pHandle->RSSetViewports(0, NULL);
		...
		if(NULL != pSwapChain)
		{
			...
			if(SUCCEEDED(hr))
			{
				...
				ID3D10RenderTargetView* pRenderTargetView = this->_CreateRenderTargetView(this->_pHandle, pSwapChain->GetHandle());

				/* if the render target view is not null */
				if(NULL != pRenderTargetView)
				{
					/* create a new depth stencil view for the swap chain */
					ID3D10DepthStencilView* pDepthStencilView = this->_CreateDepthStencilView(this->_pHandle, uiWidth, uiHeight);

					/* if the depth stencil view is not null */
					if(NULL != pDepthStencilView)
					{
						/* create a new viewport for the swap chain */
						D3D10_VIEWPORT viewport = this->_CreateViewport(0, 0, uiWidth, uiHeight);
					...
					{
						/* throw exception */
						throw NocturneDevice::Exception("Failed to create depth stencil view in ResizeSwapChain() in DX10Device");
					}
				...
				{
					/* throw exception */
					throw NocturneDevice::Exception("Failed to create render target view in ResizeSwapChain() in DX10Device");
				}
			}
			else
			{
				/* throw exception */
				throw NocturneDevice::Exception("Resizing the buffers failed in ResizeSwapChain() in DX10Device");
			}
		}
		else
		{
			/* throw exception */
			throw NocturneDevice::Exception("Invalid swap chain identifier passed to ResizeSwapChain() in DX10Device");
		}
	}
	else
	{
		/* throw exception */
		throw NocturneDevice::Exception("No device initialized to resize swap chain in ResizeSwapChain() in DX10Device");
	}
}
 ...
 * @param	uiIdentifier	The identifier of the swap chain
 */
void NocturneDeviceDX10::Device::SetActiveSwapChain(PN_SWAPCHAIN_ID uiIdentifier)
{
	...
	ID3D10RenderTargetView* pRenderTargetView = pSwapChain->GetRenderTargetView();
	...
	if(NULL != pSwapChain && NULL != pRenderTargetView)
	{
		...
		this->_pHandle->OMSetRenderTargets(1, &pRenderTargetView, pSwapChain->GetDepthStencilView());

		/* set viewport */
		this->_pHandle->RSSetViewports(1, pSwapChain->GetViewport());
	}
	else
	{
		/* throw exception */
		throw NocturneDevice::Exception("Unknown identifier of swap chain passed to set active in SetActiveSwapChain() in DX10Device");
	}
}
 ...
 * @param	uiIdentifier	The identifier of the swap chain
 */
void NocturneDeviceDX10::Device::DestroySwapChain(PN_SWAPCHAIN_ID uiIdentifier)
{
	/* unbind the resources from the device */
	this->_pHandle->OMSetRenderTargets(0, NULL, NULL);
 ...
 * @param	uiIdentifier	The identifier of the swap chain
 */
void NocturneDeviceDX10::Device::SwitchFullScreen(PN_SWAPCHAIN_ID uiIdentifier)
{
	/* get the swap chain */
	SwapChain* pSwapChain = static_cast<SwapChain*>(this->_GetSwapChain(uiIdentifier));
 ...
 */
void NocturneDeviceDX10::Device::BeginScene()
{
	/* do nothing */
}
 ...
 */
void NocturneDeviceDX10::Device::EndScene()
{
	/* do nothing */
}
 ...
 */
void NocturneDeviceDX10::Device::Clear(unsigned int uiTargets, unsigned int uiRed, unsigned int uiGreen, unsigned int uiBlue)
{
	...
	if(uiTargets & this->TARGET)
	{
		...
		this->_pHandle->ClearRenderTargetView(pSwapChain->GetRenderTargetView(), clearColor);
	}
	...
	if(uiTargets & this->DEPTHSTENCIL)
	{
		...
		this->_pHandle->ClearDepthStencilView(pSwapChain->GetDepthStencilView(), D3D10_CLEAR_DEPTH | D3D10_CLEAR_STENCIL, 1.0f, 0);
	}
}

/**
 * Create a Direct3D 10 device.
 *
 * @param	pAdapter	The IDXGIAdapter to use
 *
 * @return	Pointer to initialized device, null if it failed
 */
ID3D10Device* NocturneDeviceDX10::Device::_CreateDevice(IDXGIAdapter* pAdapter)
{
	/* declare device */
...
/**
 * Create a render target view given a D3D 10 device handle and a swap chain.
 *
 * @param	pDevice		The device handle
 * @param	pSwapChain	The swap chain to create the render target view for
 *
 * @return	Pointer to initialized ID3D10RenderTargetView, null if it failed
 */
ID3D10RenderTargetView* NocturneDeviceDX10::Device::_CreateRenderTargetView(ID3D10Device* pDevice, IDXGISwapChain* pSwapChain)
{
	...
	pSwapChain->GetBuffer(0, __uuidof(ID3D10Texture2D), reinterpret_cast<void**>(&pBackBuffer));
...
/**
 * Create a depth stencil view given an initialized ID3D10Device and a
 * width and height.
 *
 * @param	pDevice		The D3D10 device
 * @param	uiWidth		The width of the depth stencil view
 * @param	uiHeight	The height of the depth stencil view
 *
 * @return	Pointer to initialized ID3D10DepthStencilView, null if it failed
 */
ID3D10DepthStencilView* NocturneDeviceDX10::Device::_CreateDepthStencilView(ID3D10Device* pDevice, unsigned int uiWidth, unsigned int uiHeight)
{
	...
	{
#ifdef _DEBUG_
		LOGERRORALL("Cannot create depth stencil view with NULL ID3D10Device in DX10Device", "");
#endif
	}
 ...
 */
D3D10_VIEWPORT NocturneDeviceDX10::Device::_CreateViewport(unsigned int uiTopLeftX, unsigned int uiTopLeftY, unsigned int uiWidth, unsigned int uiHeight)
{
	/* declare viewport */
	D3D10_VIEWPORT viewport;

	/* set viewport properties */
	viewport.TopLeftX = uiTopLeftX;
	viewport.TopLeftY = uiTopLeftY;
	viewport.Width = uiWidth;
	viewport.Height = uiHeight;
